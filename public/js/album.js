...
function getData() {
    // URLを開く
    request.open('GET', "https://script.google.com/macros/s/AKfycbzOi_ACgcxDldiJ-8kVS5Hxxe8i37O168mubm95taRE7kAZ9a9Q/exec?page=bang-dream-album", true);
    ...
    request.onload = function() {
            ...
            console.log(data);
            console.log(data[0]);
            console.log(data[0]["CD名"]);
            var c = database.collection('bang_dream_album');
            for (let i = 0; data.length - 1 > i; i++) {
                var a = data[i];
                var d = String(i + 1);
                database.collection('bang_dream_album').doc(d).set({
                        id: Number(d),
                        name: a["CD名"],
                        artist: a["アーティスト名"],
                        type: a["タイプ"],
...
window.onload = function cdid() {
    try {
        ...
        if (typeof cdid !== "undefined") {
            database.collection("bang_dream_album").doc(cdid).get().then(function(doc) {
                ...
                if (typeof doc.data().release != "undefined") {
                    cdData += "<div class='songCoverRow'><div class='songCoverKey'>発売日</div><div class='cdCoverValue'>" + doc.data().release + "</div></div>";
                }
                if (typeof doc.data().site != "undefined") {
                    cdData += "<div class='songCoverRow'><div class='songCoverKey'>公式サイト</div><div class='cdCoverValue'><a href='" + doc.data().site + "' target='_blank'>" + doc.data().site + "</a></div></div>";
                }
                if (typeof doc.data().type != "undefined") {
                    cdData += "<div class='songCoverRow'><div class='songCoverKey'>タイプ</div><div class='cdCoverValue'>" + doc.data().type + "</div></div>";
                }
                ...
                if (typeof doc.data().broadcast != "undefined") {
            ...
            document.getElementById('songTitle').innerHTML = "BanG Dream! CD一覧";
            ...
            database.collection("bang_dream_album").orderBy("id").get().then(function(querySnapshot) {
